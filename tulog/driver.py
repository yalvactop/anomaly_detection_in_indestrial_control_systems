...
#     df_pca_var = run_pca(df, .9)# keep as many dimension as need as long as the variance is 0.9
...
#     pca_score = run_tadgan(df_pca_dim, "discardfirst21600rows_pca_dim-5" + str(rows) + "_"))
    
#     df_pca_var["Normal/Attack"] = y
#     df_pca_var["timestamp"] = timestamp
#     pca_score = run_tadgan(df_pca_var, "discardfirst21600rows_pca_var-09" + str(rows) + "_")
    
#     df_ae["Normal/Attack"] = y
#     df_ae["timestamp"] = timestamp
#     ae_score = run_tadgan(df_ae, "discardfirst21600rows_autoencoder_" + str(rows) + "_")
    
#     df_fs["Normal/Attack"] = y
#     df_fs["timestamp"] = timestamp
#     fs_score = run_tadgan(df_fs, "discardfirst21600rows_feature_selection_" + str(rows) + "_")
    
    df["Normal/Attack"] = y
    df["timestamp"] = timestamp
    bare_score = run_tadgan(df, "discardfirst21600rows_bare_" + str(rows) + "_")
